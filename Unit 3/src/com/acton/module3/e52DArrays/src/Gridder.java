package com.acton.module3.e52DArrays.src;
...
public class Gridder extends javax.swing.JFrame 
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int c = (int)(Math.random() * gridCount);
        for (int r = 0; r < gridCount; r++) {
            grid[r][c] = 1;
        }
        draw();
                    ...
                    case 0:
                        for (int x = -1; x <= 1; x++) {
                            for (int y = -1; y <= 1; y++) {
                                int nr = r + x;
                                int nc = c + y;
                                if (nr >= gridCount || nr < 0 || nc >= gridCount || nc < 0) continue;
                                if (temp[nr][nc] == 1) count++;
                            }
                        }
                        if (count >= 2) grid[r][c] = 1;
                        break;
                    case 1:
                        break;
                }
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        for (int i = 0; i < gridCount; i++) {
            for (int j = 0; j < gridCount / 2; j++) {
                int temp = grid[i][j];
                grid[i][j] = grid[i][gridCount - 1 - j];
                grid[i][gridCount - 1 - j] = temp;
            }
        }
